...
// Forma iterativa 
void changeValueInTree(int root, int node_left, int node_right, int intervalLeft, int intervalRight, int v){
    ...
    // Aqui [1,2] el 2 que es el nodo de la derecha y es menor al intervalo de la izquierda
    if( node_left > intervalRight || node_right < intervalLeft ){
        return;             //No afecta la suma                                  
    }            
    int last_in_left = (node_right+node_left)/2;

    changeValueInTree(2*root,node_left,last_in_left,intervalLeft,intervalRight,v);
    changeValueInTree(2*root+1,last_in_left+1,node_right,intervalLeft,intervalRight,v);
...
// Esta solucion causa time limited exceeded
...
int main(){
    ...
    for(int i = 0; i < q; i++){
        ...
        if(type == 1){
            ...
            cin >> u >> v;
            u--;
            // solo vamos a cambiar el numero en dentro del query u 
            changeValueInTree(1,0,n-1,u,u,v);
            // changeValueInTree(u,v,n);
        }
        else{
            int left,right;
            cin >> left >> right;
